...
class Editberita(View):
    def get(self, request,id):
        dt_berita = get_object_or_404(News,id=id,deleted_at__isnull = True)
        dt_kategori = Category.objects.filter(deleted_at__isnull = True)
        data = {
            'dt_berita' : dt_berita,
            'dt_kategori' : dt_kategori,
    ...
    def post(self, request,id):
        ...
        try:
            with transaction.atomic():
                ...
                # Find the Account by username
                # update_by = Account.objects.get(username=username)

                # Create new News object
                insert_news = get_object_or_404(News,id=id,deleted_at__isnull = True)
                insert_news.title = title
                insert_news.content = content
                insert_news.thumbnail = thumbnail 
                insert_news.slug = generate_unique_slug(insert_news)
                ...
                # Display success message and redirect
                messages.success(request, f"News '{title}' has been successfully added by {username}.")
                return redirect('admin_setori:admin_berita')
            ...
            print('Error adding News:', e)
            messages.error(request, "Failed mengedit")
            return redirect('admin_setori:admin_berita')
