...
class PotensiViews(View):
    def get(self, request):
        dt_potensi = News.objects.filter(deleted_at__isnull=True).order_by('-created_at')  # Ambil semua berita yang tidak dihapus

        paginator = Paginator(dt_berita, 4)  # Batasi per halaman
